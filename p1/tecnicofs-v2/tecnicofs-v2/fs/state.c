...
void state_init() {
    ...
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        ...
        open_file_table[i].rwlock=rwlock;
    }
...
void state_destroy() { /* nothing to do */
    ...
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_rwlock_destroy(&open_file_table[i].rwlock);
    }
...
int inode_create(inode_type n_type) {
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            pthread_rwlock_t rwlock;
            pthread_rwlock_init(&rwlock,NULL);
            inode_table[inumber].rwlock=rwlock;
...
int add_to_open_file_table(int inumber, size_t offset) {
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_rwlock_wrlock(&open_file_table[i].rwlock);
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
             pthread_rwlock_unlock(&open_file_table[i].rwlock);
            return i;
        }
    pthread_rwlock_unlock(&open_file_table[i].rwlock);
    }
